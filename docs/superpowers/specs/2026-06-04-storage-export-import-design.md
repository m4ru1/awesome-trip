...
App.tsx (state owner)
  └── useTripLibrary.ts  ← 唯一公开 API，拥有所有 key 名和持久化逻辑
        ├── storage.ts   ← envelope 包装/解包、完整性校验、容量计算
        ├── export.ts    ← 序列化为 JSON、文件名生成与清理
        └── import.ts    ← JSON 解析、Schema 校验、冲突检测
```

**核心原则**：`useTripLibrary` 是持久化的唯一入口。不创建与之平行的 service 层。storage/export/import 三个模块是内部辅助模块，由 hook 导入并编排。外部组件只通过 hook 返回值与持久化交互。
...
### 2.6 clearAllData 的 key 范围

`clearAllData()` 仅清除行程数据，不清除用户偏好和应用状态。精确清除以下 key：

| Key | 说明 |
|-----|------|
| `tt_trips_v2` | 当前行程数据 |
| `tt_trips_backup_v2` | 备份数据 |
| `tt_active_trip_v2` | 活跃行程 ID |
| `tt_trips_v1` | 旧版遗留数据 |
| `tt_active_trip_v1` | 旧版遗留活跃 ID |

**不触碰**的 key：`tt_nickname`（发布昵称）、`tt_published_v1`（发布状态）、`tt_remix_*`（remix 来源）、`tt_seeded_v1`（首次引导）、`tt_seen_help_v4`（帮助提示）、`tt_animations_v1`（动画偏好）。

调用前二次确认对话框提示："将删除所有行程数据，此操作不可撤销。建议先导出行程备份。"

### 2.7 公开方法（由 useTripLibrary 暴露）

```ts
verifyIntegrity(): { ok: boolean; error?: string }
getStorageStats(): StorageStats
clearTripData(): void     // 仅清除行程数据（上述 5 个 key）
```
...
interface ExportEnvelope {
  schemaVersion: number      // 当前 = 1，Trip 类型演变时递增，用于导入时的格式迁移
...
**文件名清理规则**：trip title 中可能包含文件系统非法字符，sanitize 时替换或删除：`/ \ : * ? " < > |` → 替换为 `-`；连续多个 `-` 合并为一个。
...
**机制：useTripLibrary 暴露 `importVersion` 计数器，App.tsx 监听其变化。**

```ts
// useTripLibrary 新增返回值
const [importVersion, setImportVersion] = useState(0)

// importTripsJSON 成功写入后：
setTrips(newTrips)
setImportVersion(v => v + 1)  // 触发 App.tsx 重载活跃 trip
```

```ts
// App.tsx 新增 useEffect
const { importVersion, getTrip, activeTripId } = tripLib
useEffect(() => {
  if (activeTripId) {
...
`setTrips` 更新 trips 列表，`importVersion` 递增通知 App.tsx 从新列表重载活跃 trip。autosave useEffect 在此期间不会干扰，因为 trips state 已经是包含导入数据的新数组。
